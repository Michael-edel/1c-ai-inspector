...
Основная модель — `gpt-5.6-luna`. Для неё встроен опубликованный тариф OpenAI на 2026-07-14: `$1` за 1M обычных входных токенов, `$0.10` за 1M кэшированных входных и `$6` за 1M выходных. При текущих лимитах консервативная верхняя оценка одного запроса составляет `66.13 KZT` по зафиксированному курсу. Тариф `gpt-5.5` сохранён для совместимости с историческими записями. Формула отдельно вычитает `cachedInputTokens` из обычного input, чтобы кэш не оплачивался дважды. Все три значения можно одновременно переопределить через `MODEL_INPUT_COST_PER_1K`, `MODEL_CACHED_INPUT_COST_PER_1K` и `MODEL_OUTPUT_COST_PER_1K`; частичное переопределение блокируется как `MODEL_PRICING_OVERRIDE_INCOMPLETE`. Для неизвестной модели без полного тарифа задача не запускается.

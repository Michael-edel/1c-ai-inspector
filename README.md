...
После запуска откройте `http://localhost:5173`. Панель показывает readiness, проекты из PostgreSQL, policy/toolset checksums, registry агентов, запускает MCP discovery и позволяет просматривать audit/report созданной task. В production frontend получает `APP_DOMAIN` и принимает запросы только для этого домена, `localhost` и `127.0.0.1`; это предотвращает ошибку Vite `Blocked request. This host is not allowed` за reverse proxy.
...
Резервная копия PostgreSQL в Windows: `.\scripts\backup.ps1 -OutputDirectory .\backups`. Для production Linux используйте `infra/ops/backup-postgres.sh`: он создает и проверяет custom-format dump для application PostgreSQL и Keycloak, хранит их в `/var/backups/1c-ai-inspector` и удаляет файлы старше `KEEP_DAYS` (по умолчанию 14). Unit `infra/ops/1c-ai-inspector-backup.timer` запускает этот backup ежедневно в 03:30 UTC. Восстановление намеренно требует явного подтверждения: `.\scripts\restore.ps1 -BackupFile .\backups\<file>.dump -ConfirmRestore`.

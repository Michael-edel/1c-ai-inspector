...
Agent retrieval принимает только явный `request.retrieval` plan, но backend нормализует параметры по исходному task text и не доверяет клиентскому `validate_query`. Каждый шаг проверяется по опубликованному read-only tool и capability конкретного агента, результат маркируется как untrusted MCP context, а вызов попадает в `tool_calls` audit. Для object-aware `search_code` backend оставляет в контексте только совпадающие заголовки модулей, чтобы большой результат поиска не вытеснял полезный код за пределы model context. Для method-only запроса сервер принимает решение о дополнительном `read_source` только по единственному модулю, в секции которого найдено точное объявление `Процедура` или `Функция`; отсутствие объявления и неоднозначный поиск возвращают разные диагностические коды и не приводят к угадыванию модуля.
...
Для запроса конкретной процедуры или функции Inspector автоматически выполняет цепочку MCP v0.9 `resolve_symbol -> get_source_checksum -> estimate_tool_payload -> read_method_source -> find_references`. До чтения исходника backend проверяет оценочный размер ответа по лимитам одной операции и всей задачи. Полный метод с исходными номерами строк передается модели только после успешной проверки; checksum и оценка размера остаются в execution audit и не расходуют контекст модели. Для аудита всего модуля сохраняется `read_source`. Структура объекта читается через компактный `get_edt_metadata_summary`; при недоступности полного набора v0.9 backend автоматически использует прежний путь `search_code -> read_method_source/read_source`.

Прочитанный `read_method_source` кэшируется в памяти worker по ключу `module + method + SHA-256`. Кэш ограничен 32 записями и 8 MB, не учитывается как новый MCP-трафик и автоматически перестает использоваться после изменения checksum модуля. В audit такой повтор отмечается причиной `SOURCE_CHECKSUM_CACHE`, отдельно от idempotent retry.

Форма задачи разделяет техническую готовность MCP и пользовательские подтверждения. После статуса `READY` поля проекта, агента и запроса доступны сразу; до отправки кнопка отдельно показывает `Подтвердите стоимость выше`, `Подтвердите расход трафика` или причину технической блокировки, а не маскирует эти состояния сообщением об ожидании MCP.
...
Текущая bridge policy версии `1.5.0` публикует шестнадцать development-инструментов. `1C Data Audit Agent` использует только `read_register_records`: bridge проверяет имя регистра, сам формирует `ВЫБРАТЬ ПЕРВЫЕ ...`, ограничивает выборку 500 строками и не принимает произвольный запрос 1С. UI запрашивает 50 строк на одну задачу, чтобы выборка помещалась в контролируемый model context. Поэтому `execute_query` и `get_event_log` по-прежнему не публикуются Inspector. Findings по данным подтверждаются evidence типа `data_row`, а отчет явно помечает ограниченную live-выборку. Остальные пятнадцать tools обслуживают BSL, метаданные, навигацию v0.9 и валидацию запросов; для idempotent `validate_query` разрешён один повтор после временного timeout.
...
Агенты v0.1: `1c_code_assistant`, `1c_query_agent`, `1c_audit_agent`, `1c_data_audit_agent`. Последний предназначен для ограниченного аудита live-записей регистров; Structured report требует непустой `evidence` для каждого finding и ссылку на объект 1С.

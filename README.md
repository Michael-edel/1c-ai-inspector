...
Для вопроса по имени процедуры или функции backend не передает весь русский текст в MCP: он извлекает имя, удаляет нерелевантный `bsl_syntax_help`, выполняет точный `search_code` и при единственном найденном объявлении автоматически добавляет `read_source` целого модуля. Ответ Code/Audit Agent не может получить статус `completed`, если запрошенное объявление не подтверждено полным исходником: сервер очищает неподтвержденные findings и сохраняет `failed` с `SOURCE_CONTEXT_INSUFFICIENT`. `1C Query Agent` принимает только текст запроса 1С, где `ВЫБРАТЬ` начинается с новой строки или следует после двоеточия; обычный вопрос блокируется до MCP-вызова кодом `AGENT_REQUEST_NOT_SUPPORTED`. Ошибки worker теперь всегда сохраняются как полный StructuredReport, поэтому `LOAD REPORT` показывает summary, limitation и следующий шаг даже для старых минимальных failure-записей.

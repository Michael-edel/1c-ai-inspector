...
Off-site копия последних application и Keycloak dumps загружается с VPS командой `.\scripts\download-production-backups.ps1`. По умолчанию файлы сохраняются в `D:\Backups\1c-ai-inspector`, каждый файл принимается только после сравнения SHA-256 с сервером, а подтверждение последнего успешного запуска записывается в `last-success.json`; срок локального хранения — 30 дней. Windows-задачу запускайте ежедневно после production backup timer, например в 09:00 по локальному времени. Локальный диск не заменяет независимое object storage: для disaster recovery добавьте вторую зашифрованную копию вне VPS и рабочего компьютера.

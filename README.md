...
Ротация bridge-токена выполняется единым fail-safe скриптом `.\scripts\rotate-mcp-bridge-token.ps1 -BridgeEnvFile C:\path\to\onec-mcp-bridge\.env`. Он генерирует новый секрет, синхронно обновляет `ONEC_MCP_BRIDGE_TOKEN` на Windows и `MCP_BRIDGE_TOKEN` на VPS, пересоздает только backend/worker, перезапускает boot-задачу bridge, проверяет авторизованный health обоих endpoints, отклонение старого токена и production monitor. При ошибке выполняется rollback; успешный токен сохраняется только в локальном credentials-файле с ограниченным ACL. Значение токена не передавайте в аргументах команд и не добавляйте в Git.
...
Production monitor проверяет health, security headers, readiness, policy, наличие минимум 8 MCP tools, синхронизированные проекты и отсутствие исходного запроса/результата в истории. Windows: `.\scripts\production-monitor.ps1 -BaseUrl https://inspector.michael.kz`. VPS: `BASE_URL=https://inspector.michael.kz /opt/1c-ai-inspector/infra/ops/monitor.sh`. Внешний workflow `.github/workflows/production-monitor.yml` выполняет ту же проверку с независимого GitHub runner каждые 15 минут и вручную через Actions; для production он требует минимум 9 обнаруженных tools. Чтобы получать сообщения о сбоях, включите GitHub Actions notifications для репозитория. Финальная acceptance-проверка выполняется единым `scripts/final-production-acceptance.ps1`, затем запускаются `infra/ops/backup-postgres.sh` и `infra/ops/restore-drill.sh`.
...
Off-site копия последних application и Keycloak dumps загружается с VPS командой `.\scripts\download-production-backups.ps1`. По умолчанию файлы сохраняются в `D:\Backups\1c-ai-inspector`, каждый файл принимается только после сравнения SHA-256 с сервером, а подтверждение последнего успешного запуска записывается в `last-success.json`; срок локального хранения — 30 дней. Второй этап `.\scripts\upload-production-backups-r2.ps1` повторно проверяет локальные SHA-256, загружает только файлы из успешного manifest в приватный Cloudflare R2 bucket и скачивает каждый объект обратно для контрольного сравнения; результат фиксируется в `r2-last-success.json`. Полный ежедневный конвейер запускается через `.\scripts\offsite-production-backup.ps1` после production backup timer. До подключения R2 доступен безопасный preflight: `.\scripts\upload-production-backups-r2.ps1 -ValidateOnly`. R2 нужно предварительно включить в Cloudflare, создать bucket `onec-ai-inspector-backups` и настроить lifecycle retention; хранение шифруется платформой Cloudflare, а публичный доступ к bucket включать не следует.

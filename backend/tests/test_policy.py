...
def test_bridge_policy_is_read_only() -> None:
    ...
    assert snapshot.policy.version == "1.5.0"
    ...
    assert snapshot.published_tools["validate_query"].idempotent is True
    assert snapshot.published_tools["validate_query"].retries == 1

...
| `MODEL_REQUEST_FAILED` | `task 200` | Модель временно недоступна. | `task_failed`; no report or findings persisted. | No automatic model retry; create a new task after recovery. | Provider exception is not returned. |

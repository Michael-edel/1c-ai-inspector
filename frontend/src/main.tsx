...
type Audit = { task_id: string; status: string; events: { type: string }[]; tool_calls: { toolName: string; mode: string; status: string; durationMs: number | null; errorCode: string | null; requestSizeBytes: number | null; resultSizeBytes: number | null }[]; model_usage: { model: string; inputTokens: number; outputTokens: number; estimatedCost: number; estimatedCostKzt: number; usdKztRate: number; durationMs: number; cachedInputTokens: number; pricingSource: string; requestSizeBytes: number; responseSizeBytes: number }[]; traffic: { mcpBytes: number; modelBytes: number; totalBytes: number } };
type TrafficStatus = { period: string; usedBytes: number; reservedBytes: number; accountedBytes: number; remainingBytes: number; warningBytes: number; criticalBytes: number; hardLimitBytes: number; level: "normal" | "warning" | "critical" | "blocked"; maxTaskBytes: number };
type ModelCostStatus = { period: string; currency: "KZT"; calls: number; inputTokens: number; cachedInputTokens: number; outputTokens: number; estimatedCostUsd: number; estimatedCostKzt: number; usdKztRate: number; rateDate: string; rateSource: string; pricing: { model: string; inputPer1M: number; cachedInputPer1M: number; outputPer1M: number; source: string } };
type ModelCostEstimate = { currency: "KZT"; estimateType: "upper-bound"; inputTokens: number; outputTokens: number; estimatedCostUsd: number; estimatedCostKzt: number; usdKztRate: number; rateDate: string; rateSource: string; pricingSource: string; model: string };
type TaskState = { taskId: string; status: string; attempt: number; resultReady: boolean; lastErrorCode: string | null; cancelRequested: boolean };
type TaskHistoryItem = { taskId: string; status: string; agentCode: string | null; agentName: string | null; projectId: string; projectName: string | null; environment: string | null; createdAt: string; updatedAt: string; resultReady: boolean; lastErrorCode: string | null; cancelRequested: boolean };
...
const isCancellableTaskStatus = (status: string) => ["created", "discovering", "analyzing", "reporting", "queued", "running"].includes(status);
const taskStatusLabels: Record<string, string> = {
  created: "Создана",
  discovering: "Получение данных",
  analyzing: "Анализ",
  reporting: "Формирование отчета",
  queued: "В очереди",
  running: "Выполняется",
  completed: "Завершена",
  failed: "Ошибка",
...
const taskStatusDescriptions: Record<string, string> = {
  created: "Задача ожидает свободного worker.",
  discovering: "MCP получает метаданные и исходный код в read-only режиме.",
  analyzing: "Модель анализирует полученный контекст.",
  reporting: "Результат проверяется и сохраняется в отчет.",
...
const publicErrorMessages: Record<string, string> = {
  ...
  NON_IDEMPOTENT_RETRY_BLOCKED: "Повтор небезопасного read-only вызова заблокирован. Создайте новую проверенную задачу.",
  ...
  MODEL_COST_CONFIRMATION_REQUIRED: "Подтвердите предварительную стоимость в тенге перед отправкой запроса модели.",
  MODEL_PRICING_OVERRIDE_INCOMPLETE: "Тариф модели настроен не полностью. Оператор должен указать все три цены.",
  MODEL_PRICING_NOT_CONFIGURED: "Для выбранной модели не настроен тариф. Запрос не отправлен.",
...
const api = async <T,>(path: string, options?: RequestInit): Promise<T> => {
  ...
  if (!response.ok) {
    ...
    try {
      ...
      // Keep the public error generic when the response is not JSON.
    }
    const message = (code && publicErrorMessages[code]) || (response.status === 409 ? "Действие недоступно для текущего состояния задачи." : "Запрос не выполнен. Повторите попытку.");
...
function App() {
  ...
  const [modelCosts, setModelCosts] = useState<ModelCostStatus | null>(null);
  const [modelCostEstimate, setModelCostEstimate] = useState<ModelCostEstimate | null>(null);
  ...
  const [rawReport, setRawReport] = useState<ReportExport | null>(null);
  const [rawReportVisible, setRawReportVisible] = useState(false);
  const [rawReportBusy, setRawReportBusy] = useState(false);
  ...
  const [patchFindingId, setPatchFindingId] = useState("");
  ...
  const refresh = async () => {
    try {
      setError("");
      const [nextReadiness, nextPolicy, nextAgents, nextTraffic, nextModelCosts, nextCostEstimate] = await Promise.all([
        api<Readiness>("/api/v1/system/readiness"),
        api<Policy>("/api/v1/system/policy"),
        api<Agent[]>("/api/v1/agents"),
        api<TrafficStatus>("/api/v1/system/traffic"),
        api<ModelCostStatus>("/api/v1/system/costs"),
        api<ModelCostEstimate>("/api/v1/system/cost-estimate"),
      ]);
      setReadiness(nextReadiness);
      setPolicy(nextPolicy);
      setAgents(nextAgents);
      setTraffic(nextTraffic);
      setModelCosts(nextModelCosts);
      setModelCostEstimate(nextCostEstimate);
      try {
  ...
  const trafficNeedsConfirmation = traffic?.level === "warning" || traffic?.level === "critical";
  const trafficBlocked = traffic?.level === "blocked";
  const mcpReady = readiness?.status === "ready";
  const canEditTask = mcpReady && !trafficBlocked;
  const canCreateTask = canEditTask && costConfirmed && (!trafficNeedsConfirmation || trafficConfirmed);
  const taskPanelStatus = !mcpReady
    ? "MCP LOCKED"
    : trafficBlocked
      ? "TRAFFIC STOP"
      : !costConfirmed
        ? "CONFIRM COST"
        : trafficNeedsConfirmation && !trafficConfirmed
          ? "CONFIRM TRAFFIC"
          : "OPEN";
  const taskActionLabel = !mcpReady
    ? "Ожидание MCP tools"
    : trafficBlocked
      ? "Лимит 9,8 GB достигнут"
      : !costConfirmed
        ? "Подтвердите стоимость выше"
        : trafficNeedsConfirmation && !trafficConfirmed
          ? "Подтвердите расход трафика"
          : "Создать read-only задачу";
  const shouldUseAuditAgent = auditAgentMismatch(selectedAgent, taskText);
  const visibleFindings = report?.persistedFindings?.length ? report.persistedFindings : report?.findings ?? [];
  useEffect(() => {
    const findings = report?.persistedFindings ?? [];
    setPatchFindingId((current) => findings.some((item) => item.id === current) ? current : findings[0]?.id ?? "");
  }, [report]);
  const historyAgentOptions = Array.from(new Map(
    taskHistory.map((item) => [item.agentCode ?? item.agentName ?? "unknown", item.agentName ?? item.agentCode ?? "Агент"]),
  ...
  const filteredTaskHistory = taskHistory.filter((item) => (
    ...
    && (historyAgentFilter === "all" || (item.agentCode ?? item.agentName ?? "unknown") === historyAgentFilter)
  ));
  const visibleTaskHistory = historyExpanded ? filteredTaskHistory : filteredTaskHistory.slice(0, 1);
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        ...
        setTaskAttempt(state.attempt);
        setTaskCancelRequested(state.cancelRequested);
        setTaskLastErrorCode(state.lastErrorCode ?? "");
        const nextAudit = await api<Audit>(`/api/v1/tasks/${taskId}/audit`);
        if (cancelled) return;
        setAudit(nextAudit);
        const [nextTraffic, nextModelCosts] = await Promise.all([api<TrafficStatus>("/api/v1/system/traffic"), api<ModelCostStatus>("/api/v1/system/costs")]);
        setTraffic(nextTraffic);
        setModelCosts(nextModelCosts);
  ...
  const createTask = async (event: React.FormEvent) => {
    ...
    try {
      const created = await api<{ task_id: string; status: string }>("/api/v1/tasks", {
        ...
        body: JSON.stringify({ projectId: selectedProject, agentId: selectedAgent, trafficConfirmed, costConfirmed, request: { text: taskText.trim(), retrieval: retrievalPlanForAgent(selectedAgent, taskText, policy?.publishedTools ?? []) } }),
      });
      setTaskId(created.task_id);
      setTaskStatus(created.status || "created");
      setTaskAttempt(0);
  ...
  const cancelTask = async () => {
    if (!taskId || !isCancellableTaskStatus(taskStatus) || cancelBusy) return;
    setCancelBusy(true);
    setError("");
    try {
      const result = await api<{ status: string; cancelRequested: boolean; lastErrorCode: string | null }>(`/api/v1/tasks/${taskId}/cancel`, { method: "POST" });
      setTaskStatus(result.status);
      setTaskCancelRequested(result.cancelRequested);
      setTaskLastErrorCode(result.lastErrorCode ?? "");
      setMessage(result.status === "cancelled" ? "Задача отменена." : "Запрос на отмену принят. Текущий вызов завершится безопасно.");
      void loadTaskHistory();
    } catch (reason) {
      setError(reason instanceof ApiError ? reason.message : "Не удалось отменить задачу.");
    } finally {
      setCancelBusy(false);
    }
  };
  ...
  const selectHistoryTask = (item: TaskHistoryItem) => {
    ...
    setTaskAttempt(0);
    setTaskCancelRequested(item.cancelRequested);
  ...
  const loadReport = async () => {
    if (!taskId || reportBusy) return;
    setReportBusy(true);
    try {
      setReport(await api<Report>(`/api/v1/tasks/${taskId}/report`));
      setMessage("Отчёт загружен ниже.");
      window.requestAnimationFrame(() => reportSectionRef.current?.scrollIntoView({ behavior: "smooth", block: "start" }));
    } catch (reason) {
  ...
  const toggleRawReport = async () => {
    if (!taskId || rawReportBusy) return;
    if (rawReportVisible) {
      setRawReportVisible(false);
      return;
    }
    setRawReportBusy(true);
    setError("");
    try {
      const payload = rawReport ?? await api<ReportExport>(`/api/v1/tasks/${taskId}/report/export`);
      setRawReport(payload);
      setRawReportVisible(true);
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : "JSON отчёта ещё не готов");
    } finally {
      setRawReportBusy(false);
    }
  };
  ...
  const createPatchProposal = async (event: React.FormEvent) => {
    event.preventDefault();
    if (!selectedProject || !patchPath.trim() || (!patchFindingId && patchOriginal === patchProposed)) return;
    if (patchFindingId && (!taskId || !authToken.trim())) {
      setError("Для импорта исходника из отчета нужен Inspector token.");
      return;
    }
    setPatchBusy(true);
    setError("");
    try {
      const fromFinding = Boolean(patchFindingId && taskId);
      const created = await api<PatchProposal>(fromFinding ? "/api/v1/patch-proposals/from-finding" : "/api/v1/patch-proposals", {
        method: "POST",
        headers: { "Content-Type": "application/json", ...(fromFinding ? { Authorization: `Bearer ${authToken.trim()}` } : {}) },
        body: JSON.stringify(fromFinding ? {
          taskId,
  ...
  return <div className="shell">
    ...
    <main className="content">
      ...
      {error && <div className="alert error">{error}</div>}{message && <div className="alert success">{message}</div>}{audit && <div className="alert traffic-task-result">Трафик выбранной задачи: <strong>{formatBytes(audit.traffic.totalBytes)}</strong> · MCP {formatBytes(audit.traffic.mcpBytes)} · модель {formatBytes(audit.traffic.modelBytes)}</div>}
      ...
      {modelCostEstimate && <section className="panel cost-confirm-panel" aria-live="polite"><div className="panel-heading"><div><span className="panel-index">PRE</span><h3>Подтверждение стоимости</h3></div><span className="panel-note">ДО ОТПРАВКИ В MODEL API</span></div><div className="cost-estimate-value"><span>Максимальная предварительная стоимость</span><strong>{formatKzt(modelCostEstimate.estimatedCostKzt)}</strong><small>{formatUsd(modelCostEstimate.estimatedCostUsd)} · до {formatTokens(modelCostEstimate.inputTokens)} входных и {formatTokens(modelCostEstimate.outputTokens)} выходных токенов</small></div><label className="cost-confirmation"><input type="checkbox" checked={costConfirmed} onChange={(event) => setCostConfirmed(event.target.checked)} />Подтверждаю отправку запроса в {modelCostEstimate.model} с лимитом до {formatKzt(modelCostEstimate.estimatedCostKzt)}</label><p className="cost-pricing">Курс: 1 USD = {modelCostEstimate.usdKztRate.toLocaleString("ru-RU")} ₸ · {modelCostEstimate.rateSource} · {modelCostEstimate.rateDate}. После выполнения будет записана фактическая сумма.</p></section>}
      ...
      {modelCosts && <section className="panel cost-panel"><div className="panel-heading"><div><span className="panel-index">₸</span><h3>Учёт MODEL API</h3></div><span className="panel-note">{modelCosts.period} · KZT</span></div><div className="cost-summary"><div><span>Расход за месяц</span><strong>{formatKzt(modelCosts.estimatedCostKzt)}</strong></div><div><span>Вызовы модели</span><strong>{formatTokens(modelCosts.calls)}</strong></div><div><span>Входные токены</span><strong>{formatTokens(modelCosts.inputTokens)}</strong></div><div><span>Выходные токены</span><strong>{formatTokens(modelCosts.outputTokens)}</strong></div></div><p className="cost-pricing"><span>Кэшировано: <strong>{formatTokens(modelCosts.cachedInputTokens)}</strong></span><span>USD: <strong>{formatUsd(modelCosts.estimatedCostUsd)}</strong></span><span>Тариф за 1M: <strong>{formatUsd(modelCosts.pricing.inputPer1M, 2)} / {formatUsd(modelCosts.pricing.cachedInputPer1M, 2)} / {formatUsd(modelCosts.pricing.outputPer1M, 2)}</strong></span><span>Курс: <strong>{modelCosts.usdKztRate.toLocaleString("ru-RU")} ₸</strong> ({modelCosts.rateDate})</span></p></section>}
      {traffic && <section className={`panel traffic-panel traffic-${traffic.level}`} aria-live="polite"><div className="panel-heading"><div><span className="panel-index">NET</span><h3>Контроль трафика</h3></div><span className="panel-note">{traffic.period} · {traffic.level.toUpperCase()}</span></div><div className="traffic-summary"><div><span>Использовано</span><strong>{formatBytes(traffic.accountedBytes)}</strong></div><div><span>Осталось до остановки</span><strong>{formatBytes(traffic.remainingBytes)}</strong></div><div><span>Максимум одной задачи</span><strong>{formatBytes(traffic.maxTaskBytes)}</strong></div></div><div className="traffic-scale"><div className="traffic-fill" style={{ width: `${Math.min(100, traffic.accountedBytes / traffic.hardLimitBytes * 100)}%` }} /><span className="traffic-marker warning-marker" title="Предупреждение 5 GB" /><span className="traffic-marker critical-marker" title="Критическое предупреждение 8 GB" /></div><div className="traffic-labels"><span>0</span><span>5 GB · предупреждение</span><span>8 GB · критично</span><span>9,8 GB · стоп</span></div>{traffic.level === "warning" && <p className="traffic-warning">Использовано более 5 GB. Перед каждой новой задачей требуется подтверждение.</p>}{traffic.level === "critical" && <p className="traffic-warning">Использовано более 8 GB. Остаток мал; новая задача требует явного подтверждения.</p>}{traffic.level === "blocked" && <p className="traffic-warning">Достигнут лимит 9,8 GB. Новые внешние вызовы остановлены до начала следующего месяца.</p>}</section>}
      <section className="panel task-history-panel" id="history"><div className="panel-heading"><div><span className="panel-index">00</span><h3>История задач</h3><span className="panel-note">{taskHistory.length} задач</span></div><div className="history-toolbar"><button className="ghost-button compact" onClick={() => setHistoryExpanded((expanded) => !expanded)}>{historyExpanded ? "СВЕРНУТЬ" : "ПОКАЗАТЬ ВСЕ"}</button><button className="ghost-button compact" onClick={() => void loadTaskHistory()}>ОБНОВИТЬ</button></div></div>{historyExpanded && <div className="history-filters" aria-label="Фильтры истории задач"><div className="history-filter"><label htmlFor="history-status">Статус</label><select id="history-status" value={historyStatusFilter} onChange={(event) => setHistoryStatusFilter(event.target.value)}><option value="all">Все статусы</option>{["created", "discovering", "analyzing", "reporting", "completed", "failed", "cancelled", "queued", "running"].map((status) => <option key={status} value={status}>{taskStatusLabel(status)}</option>)}</select></div><div className="history-filter"><label htmlFor="history-agent">Агент</label><select id="history-agent" value={historyAgentFilter} onChange={(event) => setHistoryAgentFilter(event.target.value)}><option value="all">Все агенты</option>{historyAgentOptions.map(([code, name]) => <option key={code} value={code}>{name}</option>)}</select></div><span className="history-filter-count">Показано: {filteredTaskHistory.length} из {taskHistory.length}</span></div>}{!historyExpanded && taskHistory.length > 1 && <p className="history-collapsed-note">Показана последняя задача. Нажмите «ПОКАЗАТЬ ВСЕ», чтобы открыть историю и фильтры.</p>}{visibleTaskHistory.length ? <div className="task-history-list">{visibleTaskHistory.map((item) => <button type="button" className={`task-history-row ${item.taskId === taskId ? "selected" : ""}`} key={item.taskId} onClick={() => selectHistoryTask(item)}><span className={`history-status history-${item.status}`}>{taskStatusLabel(item.status)}</span><span className="history-main"><strong>{item.agentName ?? item.agentCode ?? "Агент"}</strong><small>{item.projectName ?? item.projectId} · {formatTaskTime(item.createdAt)}{item.cancelRequested ? " · отмена запрошена" : ""}</small></span><span className="history-id">{item.taskId.slice(0, 16)}…</span></button>)}</div> : <p className="empty-note">{historyExpanded ? "По выбранным фильтрам задач нет." : "Задач пока нет."}</p>}</section>
      <section className="work-grid">
        <div className="panel task-panel">
          <div className="panel-heading">
            <div><span className="panel-index">01</span><h3>Поставить задачу</h3></div>
            <span className="lock">{taskPanelStatus}</span>
          </div>
          <form onSubmit={createTask}>
            <label htmlFor="project">EDT project</label>
            <select id="project" value={selectedProject} onChange={(event) => setSelectedProject(event.target.value)} disabled={!canEditTask || projects.length === 0}>
              <option value="">Выберите проект</option>
              {projects.map((project) => <option key={project.id} value={project.id}>{project.name} · {project.environment}</option>)}
            </select>
            <label htmlFor="agent">Agent profile</label>
            <select id="agent" value={selectedAgent} onChange={(event) => setSelectedAgent(event.target.value)} disabled={!canEditTask}>
              {agents.map((agent) => <option key={agent.code} value={agent.code}>{agent.name}</option>)}
            </select>
            <label htmlFor="task">Запрос по коду 1С</label>
            <textarea id="task" value={taskText} onChange={(event) => setTaskText(event.target.value)} placeholder="Например: проверь запросы в модуле документа ЗаказКлиента" disabled={!canEditTask} />
            <div className={`task-traffic-notice ${trafficNeedsConfirmation ? "requires-confirmation" : ""}`}>
              <span>Внешний трафик задачи: не более {formatBytes(traffic?.maxTaskBytes ?? 0)}</span>
              <small>Фактический объем появится в Execution audit.</small>
            </div>
            {trafficNeedsConfirmation && <label className="traffic-confirmation"><input type="checkbox" checked={trafficConfirmed} onChange={(event) => setTrafficConfirmed(event.target.checked)} />Подтверждаю запуск с учетом текущего расхода {formatBytes(traffic?.accountedBytes ?? 0)}</label>}
            <button className="primary-button" disabled={!canCreateTask || !selectedProject || !taskText.trim()}>{taskActionLabel}</button>
          </form>
        </div>
      ...
      <section className="panel agents-panel"><div className="panel-heading"><div><span className="panel-index">06</span><h3>Execution audit</h3></div><div className="heading-actions"><button className="ghost-button compact" onClick={() => void loadAudit()} disabled={!taskId}>REFRESH AUDIT</button><button className="ghost-button compact" onClick={() => void loadReport()} disabled={!taskId || reportBusy}>{reportBusy ? "LOADING..." : "LOAD REPORT"}</button><button className="ghost-button compact" onClick={() => void toggleRawReport()} disabled={!taskId || rawReportBusy}>{rawReportBusy ? "LOADING JSON..." : rawReportVisible ? "СКРЫТЬ JSON" : "ПОКАЗАТЬ JSON"}</button><button className="ghost-button compact" onClick={() => void exportReport()} disabled={!taskId}>EXPORT JSON</button></div></div>{taskId ? <dl className="data-list"><div><dt>Task</dt><dd>{taskId.slice(0, 18)}…</dd></div><div><dt>Status</dt><dd>{audit?.status ?? taskStatusLabel(taskStatus)}</dd></div><div><dt>Attempt</dt><dd>{taskAttempt || "—"}</dd></div><div><dt>Cancel</dt><dd className={taskCancelRequested ? "warning" : "safe"}>{taskCancelRequested ? "requested" : "not requested"}</dd></div><div><dt>Events</dt><dd>{audit?.events.length ?? 0}</dd></div><div><dt>MCP calls</dt><dd>{audit?.tool_calls.length ?? 0}</dd></div><div><dt>Model</dt><dd>{audit?.model_usage[0]?.model ?? report?.modelUsage?.model ?? report?.execution?.modelName ?? "—"}</dd></div><div><dt>Model duration</dt><dd>{audit?.model_usage.reduce((sum, item) => sum + item.durationMs, 0) ?? 0} ms</dd></div><div><dt>Стоимость модели</dt><dd>{formatKzt(audit?.model_usage.reduce((sum, item) => sum + item.estimatedCostKzt, 0) ?? 0)}</dd></div><div><dt>Токены модели</dt><dd>{formatTokens(audit?.model_usage.reduce((sum, item) => sum + item.inputTokens + item.outputTokens, 0) ?? 0)}</dd></div><div><dt>Курс расчета</dt><dd>{audit?.model_usage[0]?.usdKztRate ? `${audit.model_usage[0].usdKztRate.toLocaleString("ru-RU")} ₸ / USD` : "—"}</dd></div>{report && <><div><dt>Summary</dt><dd>{report.summary}</dd></div><div><dt>Findings</dt><dd>{report.persistedFindings.length}</dd></div></>}</dl> : <p className="empty-note">Создайте задачу после прохождения readiness gate.</p>}{taskId && audit?.tool_calls.length ? <div className="tool-call-list"><div className="card-label">TOOL CALLS</div>{audit.tool_calls.map((call, index) => <div className="tool-call-row" key={`${call.toolName}-${index}`}><div><strong>{call.toolName}</strong><small>{call.mode} · {call.durationMs ?? "—"} ms{call.errorCode ? ` · ${call.errorCode}` : ""}</small></div><span className={`tool-call-status ${call.status}`}>{call.status}</span></div>)}</div> : null}</section>
      {report && <section ref={reportSectionRef} className="panel report-panel" id="report"><div className="panel-heading"><div><span className="panel-index">07</span><h3>Report details</h3></div><span className="panel-note">{report.status.toUpperCase()}</span></div><p className="report-summary">{report.summary}</p><div className={`report-coverage coverage-${report.sourceCoverage ?? "unknown"}`}>Контекст источника: {sourceCoverageLabels[report.sourceCoverage ?? "unknown"]}</div><div className="report-facts"><div><span>Объектов проверено</span><strong>{report.objectsReviewed?.length ?? 0}</strong></div><div><span>MCP вызовов</span><strong>{report.toolUsage?.calls ?? 0}</strong></div><div><span>Время tools</span><strong>{report.toolUsage?.durationMs ?? 0} ms</strong></div><div><span>Токены модели</span><strong>{(report.modelUsage?.inputTokens ?? 0) + (report.modelUsage?.outputTokens ?? 0)}</strong></div><div><span>Фактическая стоимость</span><strong>{formatKzt(report.modelUsage?.estimatedCostKzt ?? 0)}</strong></div><div><span>Read-only</span><strong>{report.validation?.readOnly === true ? "Да" : "—"}</strong></div></div>{report.validation?.sourceEvidence && <div className="report-validation"><strong>Проверка evidence:</strong><span>Статус: {report.validation.sourceEvidence.status ?? "—"}</span><span>Отклонено диапазонов: {report.validation.sourceEvidence.invalidCount ?? 0}</span><span>Очищено excerpts: {report.validation.sourceEvidence.excerptCount ?? 0}</span><span>Исключено findings: {report.validation.sourceEvidence.droppedFindings ?? 0}</span></div>}{report.limitations?.length ? <div className="report-limitations"><strong>Ограничения:</strong>{report.limitations.map((item, index) => <span key={`${item}-${index}`}>{item}</span>)}</div> : null}{report.nextActions?.length ? <div className="report-limitations report-actions"><strong>Следующие действия:</strong>{report.nextActions.map((item, index) => <span key={`${item}-${index}`}>{item}</span>)}</div> : null}{visibleFindings.length ? <div className="report-findings">{visibleFindings.map((finding, index) => <article className="finding-card" key={`${finding.objectFqn}-${finding.lineStart ?? index}-${index}`}><div className="finding-head"><div><span className="card-label">FINDING {index + 1}</span><strong>{finding.category}</strong></div><span className={`finding-severity ${finding.severity}`}>{finding.severity}</span></div><p className="finding-description">{finding.description}</p><div className="finding-meta"><span>Object: {finding.objectFqn}</span><span>Module: {finding.module}</span><span>Method: {finding.method ?? "—"}</span><span>Lines: {finding.lineStart ?? "—"}{finding.lineEnd && finding.lineEnd !== finding.lineStart ? `–${finding.lineEnd}` : ""}</span><span>Confidence: {Math.round(finding.confidence * 100)}%</span></div><p><strong>Risk:</strong> {finding.risk}</p><p><strong>Recommendation:</strong> {finding.recommendation}</p><div className="evidence-list"><span className="card-label">EVIDENCE</span>{finding.evidence.map((evidence, evidenceIndex) => <div className="evidence-card" key={`${evidence.type}-${evidenceIndex}`}><div>{evidence.type} · {evidence.objectFqn} · {evidence.module}{evidence.lineStart ? ` · строки ${evidence.lineStart}${evidence.lineEnd && evidence.lineEnd !== evidence.lineStart ? `–${evidence.lineEnd}` : ""}` : ""}</div>{evidence.description && <p>{evidence.description}</p>}{evidence.excerpt && <pre>{evidence.excerpt}</pre>}</div>)}</div></article>)}</div> : <p className="empty-note">Подтверждённых findings нет.</p>}</section>}
      {report?.execution && <section className="panel report-version-panel"><div className="panel-heading"><div><span className="panel-index">08</span><h3>Execution snapshot</h3></div><span className="panel-note">VERSIONS</span></div><div className="report-versions"><div><span>Prompt</span><strong>{report.execution.promptVersion}</strong></div><div><span>Model</span><strong>{report.execution.modelProvider} / {report.execution.modelName}</strong></div><div><span>Policy</span><strong>{report.execution.policyVersion}</strong></div><div><span>Policy checksum</span><strong title={report.execution.policyChecksum}>{shortChecksum(report.execution.policyChecksum)}</strong></div><div><span>Toolset checksum</span><strong title={report.execution.toolsetChecksum}>{shortChecksum(report.execution.toolsetChecksum)}</strong></div></div></section>}
      {rawReportVisible && rawReport && <section className="panel raw-report-panel" id="raw-report"><div className="panel-heading"><div><span className="panel-index">09</span><h3>Machine JSON</h3></div><span className="panel-note">REPORT + AUDIT</span></div><pre className="raw-json-view">{JSON.stringify(rawReport, null, 2)}</pre></section>}
    </main>
  </div>;
}

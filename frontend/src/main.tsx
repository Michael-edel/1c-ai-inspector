...
function App() {
  ...
  const [patchRole, setPatchRole] = useState("maintainer");
  ...
  const runPatchAction = async (action: "impact" | "revalidate" | "validate" | "checkpoint" | "approve" | "reject") => {
    ...
    try {
      ...
      if (action === "revalidate") {
        ...
        options.headers = { "Content-Type": "application/json" };
        options.body = JSON.stringify({ actor: patchActor.trim(), role: patchRole, note: patchNote.trim() });
      }
  ...
  const exportPatchPackage = () => {
    if (!patchProposal) return;
    window.open(`/api/v1/patch-proposals/${patchProposal.id}/package`, "_blank", "noopener,noreferrer");
  };
  ...
  return <div className="shell">
    ...
    <main className="content">
      ...
      <section className="panel patch-validation-bar"><div className="panel-heading"><div><span className="panel-index">05</span><h3>Validation gate</h3></div><span className="panel-note">SOURCE + DIFF</span></div><p className="panel-intro">Approval разрешен только после source revalidation и детерминированной проверки diff.</p><div className="patch-actions"><span className="panel-note">{patchProposal ? `source: ${patchProposal.sourceValidationStatus} · patch: ${patchProposal.validationStatus}` : "Создайте proposal"}</span><label htmlFor="patch-role">Decision role</label><select id="patch-role" value={patchRole} onChange={(event) => setPatchRole(event.target.value)}><option value="reviewer">reviewer</option><option value="maintainer">maintainer</option><option value="owner">owner</option></select><button type="button" className="ghost-button compact" onClick={() => void runPatchAction("validate")} disabled={patchBusy || !patchProposal || patchProposal.sourceValidationStatus !== "valid"}>VALIDATE PATCH</button></div></section>

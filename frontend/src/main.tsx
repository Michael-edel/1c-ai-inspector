...
type Audit = { task_id: string; status: string; events: { type: string }[]; tool_calls: { toolName: string; mode: string; status: string; durationMs: number | null; errorCode: string | null; requestSizeBytes: number | null; resultSizeBytes: number | null }[]; model_usage: { model: string; estimatedCost: number; durationMs: number; cachedInputTokens: number; requestSizeBytes: number; responseSizeBytes: number }[]; traffic: { mcpBytes: number; modelBytes: number; totalBytes: number } };
...
function App() {
  ...
  const refresh = async () => {
    try {
      setError("");
      const [nextReadiness, nextPolicy, nextAgents, nextTraffic] = await Promise.all([
  ...
  const canCreateTask = readiness?.status === "ready" && !trafficBlocked;
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        ...
        if (cancelled) return;
        setAudit(nextAudit);
        setTraffic(await api<TrafficStatus>("/api/v1/system/traffic"));
  ...
  const createTask = async (event: React.FormEvent) => {
    ...
    try {
      const created = await api<{ task_id: string; status: string }>("/api/v1/tasks", {
        ...
        body: JSON.stringify({ projectId: selectedProject, agentId: selectedAgent, trafficConfirmed, request: { text: taskText.trim(), retrieval: retrievalPlanForAgent(selectedAgent, taskText, policy?.publishedTools ?? []) } }),
      });

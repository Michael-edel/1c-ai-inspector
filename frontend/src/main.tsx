...
function App() {
  ...
  return <div className="shell">
    ...
    <main className="content">
      ...
      <section className="panel patch-validation-bar"><div className="panel-heading"><div><span className="panel-index">05</span><h3>Validation gate</h3></div><span className="panel-note">SOURCE + DIFF</span></div><p className="panel-intro">Approval разрешен только после source revalidation и детерминированной проверки diff.</p><div className="patch-actions"><span className="panel-note">{patchProposal ? `source: ${patchProposal.sourceValidationStatus} · patch: ${patchProposal.validationStatus}` : "Создайте proposal"}</span><button type="button" className="ghost-button compact" onClick={() => void runPatchAction("validate")} disabled={patchBusy || !patchProposal || patchProposal.sourceValidationStatus !== "valid"}>VALIDATE PATCH</button></div></section>
